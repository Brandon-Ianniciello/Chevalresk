...
class JoueurTDG extends DBAO
{
    ...
    public function get_gains_by_alias($alias){
        try {
            $conn = $this->connect();
            $tableName = $this->tableName;
            $query = "SELECT montantInitial FROM $tableName WHERE alias =:alias";
            $stmt = $conn->prepare($query);
    ...
    public function update_photo_by_id($url,$id){
        try {
            $conn = $this->connect();
            $tableName = $this->tableName;
            $query = "UPDATE $tableName SET urlPhotoProfil=:urlPhotoProfil WHERE idJoueur=:idJoueur";
            $stmt = $conn->prepare($query);
            $stmt->bindParam(':urlPhotoProfil', $url);
            $stmt->bindParam(':idJoueur', $id);
            $stmt->execute();
            $resp = true;
        } catch (PDOException $e) {
