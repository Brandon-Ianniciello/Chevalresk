...
class Joueur
{
    ...
    {
        $TDG = JoueurTDG::getInstance();
        $gains = $TDG->get_gains_by_alias($alias);

        if($montant < $gains['montantInitial']){
            return false;
        }
    ...
    public function update_user_montant_by_id($montant, $id)
    {
        $TDG = JoueurTDG::getInstance();
        $res = $TDG->update_montant_joueur_by_id($montant,$id);
        $this->montantInitial = $montant;
        $TDG = null;
        return $res;
    }
    ...
    public function update_user_photo($url,$id){
        $TDG = JoueurTDG::getInstance();
        $res = $TDG->update_photo_by_id($url,$id);
        $TDG = null;
        return $res;
    }
}
