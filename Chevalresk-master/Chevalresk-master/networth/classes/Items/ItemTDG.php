...
class itemTDG extends DBAO{
    ...
    public function get_all_by_moyenne($nbrÉtoiles)
    {
        try {
            $conn = $this->connect();
            $tableName = $this->tableName;
            $query = "SELECT idItem,quantiteStockItems,photoItem,nomItem,prixItem,typeItem,ROUND(moyenneÉvaluation,0)
            FROM $tableName WHERE Round(moyenneÉvaluation) = $nbrÉtoiles";
            $stmt = $conn->prepare($query);
            $stmt->execute();
            $stmt->setFetchMode(PDO::FETCH_ASSOC);
            $result = $stmt->fetchAll();
    ...
    public function update_moyenne_évaluation_by_id($idItem,$moyenne){
        try {
            $conn = $this->connect();
            $tableName = $this->tableName;
            $query = "UPDATE $tableName SET moyenneÉvaluation=$moyenne WHERE idItem=$idItem";
            $stmt = $conn->prepare($query);
            $stmt->execute();
            $resp = true;
        } catch (PDOException $e) {
